...
/// The procedural editor's own setter — the head-shape/eyes/mouth/headwear
/// fields exposed in-app (`Sprite::Procedural`'s existing fields).
#[tauri::command]
fn character_library_set_sprite_procedural(
    ...
    update_character_library(&id, |text| {
        character::set_sprite_procedural(text, &head_shape, &eyes, &mouth, &headwear)
    })
}
