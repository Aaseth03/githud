...
describe("length", () => {
  it("leaves a short message untouched", () => {
    expect(trimForSpeech("Short.")).toBe("Short.");
  });

  it("cuts a long one on a sentence boundary", () => {
    const text = ("This is a sentence that goes on. ").repeat(60);
    const got = trimForSpeech(text);

    expect(got.length).toBeLessThanOrEqual(SPOKEN_LIMIT);
    expect(got.endsWith(".")).toBe(true);
  });

  it("still cuts when there is no sentence boundary to find", () => {
    const got = trimForSpeech("x".repeat(SPOKEN_LIMIT * 2));
    expect(got.length).toBeLessThanOrEqual(SPOKEN_LIMIT);
  });

  it("strips before trimming, so a code-heavy message is not cut to nothing", () => {
    const got = prepareSpeech(
      "```\n" + "code\n".repeat(300) + "```\nThe short answer is yes.",
    );

    expect(got).toBe("The short answer is yes.");
  });
});
