...
| `layered` | **In progress** | one PNG per named part | CSS transforms + springs | D21. Continuous motion, scripted, no dependency, fully automatable pipeline (M7.5). |
...
2. **Rigging is a manual Cubism session per character** and cannot be scripted,
   which collides with principle 4 and with the M7.5 pipeline. Viable as a
