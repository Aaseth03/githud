...
- Every directory has a `CONTEXT.md` **except** `src/` subtrees generated by
  tooling. When you add a document or a path to a directory, update that
  directory's `CONTEXT.md` in the same change.
- Every `CONTEXT.md` contains an ASCII tree (`├ ─ └ │`) in a `text` code block
  mapping everything actually in that directory, plus a Routing table saying
  what each entry contains and when to use it.
- **Every tree must match disk.** Do not document a folder that does not exist;
  do not leave a real folder undocumented. A real but empty folder is kept with
  a `.gitkeep`.
...
Stable contracts. Internalize them as constraints — do not copy their content
into other files.

| Doc | Canonical for |
|---|---|
| `planning/architecture/event-schema.md` | The normalized agent event stream |
| `planning/architecture/adapter-contract.md` | The `AgentAdapter` / `AgentSession` traits |
| `planning/architecture/data-layout.md` | What is committed vs. local state |
